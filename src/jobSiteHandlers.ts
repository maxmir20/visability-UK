// Job site factory interface
interface JobSiteHandler {
  canHandle(url: string): boolean;
  extractCompanyName(url: string): string | null;
  findCompanyOnWebPage(): string | null;
}

// Format company name from LinkedIn URL
export function formatCompanyName(url: string): string | null {
...
// Indeed handler
export class IndeedHandler implements JobSiteHandler {
  canHandle(url: string): boolean {
...
// CharityJob handler
export class CharityJobHandler implements JobSiteHandler {
  canHandle(url: string): boolean {
...
export class JobSiteFactory {
  private handlers: JobSiteHandler[] = [
    new LinkedInHandler(),
    new IndeedHandler(),
    new CharityJobHandler()
  ];

  getHandler(url: string): JobSiteHandler | null {
  ...
  findCompanyOnWebPage(url: string): string | null {
    const handler = this.getHandler(url);
    if (handler) {
